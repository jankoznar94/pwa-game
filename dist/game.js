(function() {
    ...
    if (testMode) {
      ...
      MONSTER_DB.forEach(themeMonsters => {
        themeMonsters.forEach(m => {
          if (m.face && !allMonsterFaces.includes(m.face)) allMonsterFaces.push(m.face);
        });
      });
      LOCATIONS.forEach(loc => {
        if (loc && loc.boss && loc.boss.face && !allMonsterFaces.includes(loc.boss.face)) {
          allMonsterFaces.push(loc.boss.face);
        }
      });
      state.encounteredMonsters = allMonsterFaces;
      // Odemknout všechny achievementy
  ...
  function repeatRound() {
    ...
    if (mb._bonusStartTimer) { clearTimeout(mb._bonusStartTimer); mb._bonusStartTimer = null; }
    if (mb._bonusEndTimer) { clearTimeout(mb._bonusEndTimer); mb._bonusEndTimer = null; }
    // Kompletní reset patra — životy, energie, nepřítel, timery, UI
    startLocation(mb.locId);
  }
  ...
  function playSequenceAttack() {
    ...
    const bMs = Math.round(winTime * 0.15); // 15% šířka, stabilní
