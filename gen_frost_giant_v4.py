#!/usr/bin/env python3
"""Generate frost_giant as a snow giant / yeti — concrete character like Djinn."""
...
COLOR PALETTE: SNOW — white fur, pale blue-grey skin, dark eyes, frosty breath, silver accents

MONSTER: SNOW GIANT (Yeti) — massive yeti-like giant, thick white fur covering the head and shoulders, pale blue-grey skin visible on the face, two small dark eyes with an intense predatory gaze, a large flat nose, slightly open mouth showing sharp fangs, fur is matted with frost and snow, ears hidden under thick fur, a brutal and savage expression, like the abominable snowman"""
...
r = subprocess.run(["curl", "-s", "--max-time", "600",
    ...
    "--output", "/tmp/frost_giant_v4.json"], capture_output=True, text=True, timeout=620)

with open("/tmp/frost_giant_v4.json") as f:
    data = json.load(f)

b64 = data["data"][0]["b64_json"]
img_bytes = base64.b64decode(b64)
with open("/tmp/frost_giant_v4.png", "wb") as f:
    f.write(img_bytes)
print(f"Generated: {len(img_bytes)} bytes")

img = Image.open("/tmp/frost_giant_v4.png")
