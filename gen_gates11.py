#!/usr/bin/env python3
"""Generate gates at 6:1 aspect ratio (1024x170)."""
...
GATES = [
    {
        ...
        "prompt": "Warcraft 3 painting style, wide horizontal view of an ancient sandstone fortress gate, heavy iron padlock with golden pharaonic lock mechanism, scarab and ankh carvings, wide stone walls on both sides, desert theme, painted fantasy art, dramatic lighting, rich colors, gritty texture, solid black background, no text, no UI"
    },
    {
...
for gate in GATES:
    ...
    # Resize to game size (1080x180)
    final = img.resize((1080, 180), Image.LANCZOS)
    final.save(out_path, "PNG")
    
    # Preview
    preview = final.copy()
    preview.thumbnail((540, 90), Image.LANCZOS)
    preview.save(out_path.replace(".png", "_preview.png"), "PNG")
    
    print(f"  Saved {out_path} (1080x180)")
    time.sleep(3)
