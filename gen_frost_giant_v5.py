#!/usr/bin/env python3
"""Generate frost_giant as a snow yeti — pure grey-white, no blue."""
...
r = subprocess.run(["curl", "-s", "--max-time", "600",
    ...
    "--output", "/tmp/frost_giant_v5.json"], capture_output=True, text=True, timeout=620)

with open("/tmp/frost_giant_v5.json") as f:
    data = json.load(f)

b64 = data["data"][0]["b64_json"]
img_bytes = base64.b64decode(b64)
with open("/tmp/frost_giant_v5.png", "wb") as f:
    f.write(img_bytes)
print(f"Generated: {len(img_bytes)} bytes")

img = Image.open("/tmp/frost_giant_v5.png")
